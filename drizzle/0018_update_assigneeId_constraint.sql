...
CREATE TABLE `__new_reservation` (
	...
	FOREIGN KEY (`assigneeId`) REFERENCES `staff`(`userId`) ON UPDATE no action ON DELETE set null
);
--> statement-breakpoint
INSERT INTO `__new_reservation`("id", "reservationNumber", "petId", "userId", "createdAt", "date", "timeFrom", "timeTo", "status", "service", "assigneeId") SELECT "id", "reservationNumber", "petId", "userId", "createdAt", "date", "timeFrom", "timeTo", "status", "service", "assigneeId" FROM `reservation`;--> statement-breakpoint
