...
test("should create a reservation and verify the response time", async () => {
  const newReservation: Omit<CreateReservation, "id" | "createdAt" | "status"> =
    {
      petId: 11,
      userId: 12,
...
test("should delete the reservation and ensure it no longer exists", async () => {
  ...
  // Try to fetch the deleted reservation.
  const deletedReservation = await service.getById(currentReservation);
  // Assuming getById returns a failed result or undefined data when not found.
  expect(deletedReservation.data).toBeUndefined();
  // Alternatively, if getById returns a result with undefined data:
  // expect(deletedReservation.data).toBeUndefined();
});
